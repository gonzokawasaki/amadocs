...
class DocSummary {
  ...
  constructor({ model = null, basePath = null } = {}) {
    ...
    if (this.cloud) {
      // Callers pass local tags (SUMMARY/OLLAMA prefs resolved server-side) that
      // don't exist on OpenRouter. OpenRouter ids always contain a "/", so ignore
      // anything else and fall through to the cloud prefs.
      this.model =
