      ...
      this.client
        .chat({
          ...
          options: {
            ...
            // AMAdocs: hard answer-length cap. This is a search tool — answers must
            // be ~a paragraph, not an essay. ~200 tokens ≈ 120 words; the model
            // physically cannot ramble. The UI trims the trailing partial sentence.
            num_predict: 200,
          },
        })
        .then((res) => {
    ...
    const measuredStreamRequest = await LLMPerformanceMonitor.measureStream({
      func: this.client.chat({
        ...
        options: {
          temperature,
          num_ctx: this.promptWindowLimit(),
          // AMAdocs: hard answer-length cap (see getChatCompletion). ~200 tokens
          // ≈ 120 words; keeps streamed answers to ~a paragraph for this search tool.
          num_predict: 200,
        },
      }),
      messages,
